"""Эксперимент 6: быстрый decode через GEMM-селекцию (cuBLAS), без поэлементных op.

Ключевой инсайт из exp4: у Transformer `q @ k.T` — fused cuBLAS GEMM (оптимизирован
по памяти), а STS-селекция `(en * qn).sum(-1)` — поэлементное умножение + reduce
(медленнее, лишние проходы по памяти). Заменяем на `sim = en @ qn` (matrix-vector
через cuBLAS) — та же математика.

Плюс: блоки по K=256 (доказано exp5a: ΔPPL −0.0%).

Запуск: cd phase01/exp_vq && C:/Python313/python.exe exp_gemm_fast_decode.py
"""
...
def gemm_fast_decode(model, en, e_all, q0, Wc, k_eff, topk, nq, K):
    """en: (Wc,d) нормированные ключи (кэш). e_all: (Wc,d). q0: (1,d)."""
    q = q0
    ...
    for blk in model.blocks:
        ...
        sim = en @ qn.squeeze(0)            # (Wc,) GEMM — cuBLAS
        sim[Wc - TAIL:] = -1e9
        kk = min(topk, Wc - TAIL)
...
def main():
    ...
    # --- 1. Качество GEMM+fast (K=256) vs exact ---
    Wq = 65536
    print(f"Качество GEMM+fast (K={K}) на 8 окнах W={Wq}...", flush=True)
    ...
    with torch.no_grad():
        for wi in range(8):
            ...
            lg = gemm_fast_decode(model, en, e_all, q0, Wq, k_eff, topk, nq, K)
    ...
    print(f"\nЗамер скорости (GEMM+fast vs TF KV):", flush=True)
    o = 5000
    results = {}
    for Wc in [16384, 65536, 262144]:
        ...
        en = e_all / (e_all.norm(dim=-1, keepdim=True) + 1e-6)
        q0 = e_all[-nq:].mean(0, keepdim=True)
        t_sts = timeit(lambda: gemm_fast_decode(model, en, e_all, q0, Wc, k_eff, topk, nq, K),
                       warmup=2, iters=5)
        sts_tps = 1 / t_sts
    ...
    out = {"K": K, "cos": round(cos_m, 6), "delta_ppl_pct": round(dppl, 1), "results": results}
    with open(os.path.join(RESULTS, "exp_gemm_fast_decode.json"), "w", encoding="utf-8") as f:
        json.dump(out, f, indent=1)
    print(f"\nSaved: results/exp_gemm_fast_decode.json")
