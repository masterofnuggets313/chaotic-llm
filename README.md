...
пере-замере я сам их поправил — retrieval-преимущество оказалось ×1.6, а не ×2.6.
...
статьи, которых я не читал. Есть код, который проверяется smoke-тестом за 10
...
хранилось бы в памяти; я не храню состояние вовсе.
...
### Fracode Memory: как я убрал состояние из памяти
...
(замер на моей модели, не теорема). Следствие: состояние не содержит
