...
**Ключевое открытие — тождество h = G(e):** состояние h на каждом слое
детерминированно восстанавливается из статичных ключей e (cos(h_last_exact,
h_full[-1]) = 0.99999994). Следствие: состояние не содержит информации
сверх ключей — его не нужно хранить, достаточно хранить **коды ключей**
и разворачивать состояние теми же блоками модели.
