...
def build_lex():
    ...
    with _LEX_LOCK:
        ...
        lex = {}
        for m in re.finditer(r'[A-Za-zА-Яа-яЁё0-9_][A-Za-zА-Яа-яЁё0-9_\-]{3,}', text):
            a, b = m.span()
...
def lex_find(qtext, k, win=400):
    ...
    if not lex:
        return []
    pts = []   # (pos, weight)
    for w in {x.lower() for x in re.findall(r'[A-Za-zА-Яа-яЁё0-9_][A-Za-zА-Яа-яЁё0-9_\-]{3,}', qtext)}:
        a = lex.get(w)
        if a is None or len(a) == 0 or len(a) > 50_000:
            continue
    ...
    if not pts:
        return []
    pts.sort()
    # группируем в кластеры по win; репрезентант = позиция САМОГО РЕДКОГО слова
    # (иначе сниппет якорится на левую границу и игла выпадает за ±window)
    acc = []   # (rep_pos, total_weight)
    clus = []
    for p, wt in pts:
        if clus and p - clus[0][0] > win:
            tw = sum(w for _, w in clus)
            rep = max(clus, key=lambda x: x[1])[0]   # самая редкая позиция
            acc.append((rep, tw))
            clus = []
        clus.append((p, wt))
    if clus:
        ...
        acc.append((rep, tw))
    ranked = sorted(acc, key=lambda x: -x[1])[:k]
...
def link(body: LinkIn):
    ...
    if body.src not in ids or body.dst not in ids:
        raise HTTPException(404, 'нет записи')
    st['edges'].setdefault(str(body.src), [])
    if body.dst not in st['edges'][str(body.src)]:
        st['edges'][str(body.src)].append(body.dst)
    save_store()
    return {'edge': f'{body.src} -> {body.dst}'}
...
def chain(start_id: int, max_hops: int = 8):
    st = load_store(); seen, cur, path = set(), start_id, []
    while cur is not None and cur not in seen and len(path) < max_hops:
        seen.add(cur)
...
def _sts_embed_table():
    ...
    if _STS_EMB is None:
        import torch as T
        sd = T.load(os.path.join(os.path.dirname(os.path.abspath(__file__)),
                       '..', '..', 'results', 'ckpts', 'sts_prog_seed0.pt'),
                    map_location='cpu')
        _STS_EMB = sd['embed.weight'].numpy()
    return _STS_EMB
...
def _archive_tok():
    ...
    if _BPE is None:
        ...
        if tf:
            ...
            head = fb.load_chars(os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                   '..', 'corpus_public.txt'), 990_000)
            _BPE = fb.make_bpe(head)
    return _BPE
...
class RecallIn(BaseModel):
    ...
    mode: str = 'auto'    # auto | vector | lex (лексический exact-слой)
...
def recall(body: RecallIn):
    ...
    import numpy as np
    tkn = _archive_tok()
    mode = getattr(body, 'mode', 'auto')   # auto | vector | lex
    ...
    if mode in ('auto', 'vector') and ids_q:
        E = _sts_embed_table()
        q = E[ids_q].mean(0)   # запрос = сырое среднее emb окна (фронт как у ключей)
