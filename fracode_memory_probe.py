...
ИДЕЯ: сжимать не МОДЕЛЬ, а ПАМЯТЬ (ключевую таблицу (W,d)),
...
# ---------------------------------------------------------------- k-means
def kmeans(X, K, iters=15, seed=0, chunk=65536, det=True):
    """Lloyd's на GPU через matmul-трюк (без (N,K,D) тензоров). X: (N,D).

    N < K (короткий корпус: строк меньше, чем центроидов) — валидный случай:
    раньше здесь был баг. `X[randperm(N)[:K]]` давал N строк, а `sums`/`cnts`
    создавались на K, и `C[nz] = sums[nz]/cnts[nz]` падал с IndexError
    «shape of the mask [K] does not match [N]». Теперь при N < K берём индексы
    с возвращением — форма C остаётся (K, D_), лишние центроиды суть дубли.

    ДЕТЕРМИНИЗМ (11.09.2026). Раньше суммы по кластерам копились через
    `index_add_` — на CUDA это atomicAdd, порядок сложения float не определён.
    Два прогона одного и того же сида давали РАЗНЫЙ кодбук, а значит разный PPL
    (измеренный разброс Δ до 0.0342 нат при пороге вердикта 0.05 — см.
    FRAKOD_STATUS_2026-09-11_v2.md §11). Теперь при det=True суммы считаются
    через stable-сортировку по назначениям + cumsum: результат один и тот же
    от прогона к прогону. det=False оставлен только для сравнения/отладки.
    """
    N, D_ = X.shape
    dev = X.device
    if N == 0:
        return torch.zeros(K, D_, device=dev, dtype=X.dtype)
    g = torch.Generator(device=dev).manual_seed(seed)
    if N >= K:
        sel = torch.randperm(N, generator=g, device=dev)[:K]
    else:
        sel = torch.randint(0, N, (K,), generator=g, device=dev)
    C = X[sel].clone()
    Cn2 = C.pow(2).sum(1)
    for _ in range(iters):
        ...
        nz = torch.bincount(assign, minlength=K) > 0
        if det:
            # Детерминированный segment-sum: границы кластеров в отсортированном
            # порядке, суммы — разности префиксных сумм. Никаких атомиков.
            #
            # ТОЧНОСТЬ (важно, 11.09.2026). Наивный `cumsum(X)` даёт катастрофическую
            # потерю точности: префикс растёт как N*mean, а сумма кластера (~N/K строк)
            # получается вычитанием двух БОЛЬШИХ чисел. При N=8192 относительная ошибка
            # центроида ~1e-3 — это ХУЖЕ, чем у атомарного index_add_ (там копится
            # ~N/K слагаемых, ошибка ~1e-6). Из-за этого «детерминированный» кодбук
            # уезжал от атомарного ДАЛЬШЕ, чем два атомарных друг от друга
            # (наблюдали 0.045 нат против 0.004 — FRAKOD_STATUS §13.3).
            # Лечение: (1) вычитаем среднее по столбцу — префикс растёт как sqrt(N)*std,
            # а не N*mean; (2) копим в float64. Тогда ошибка ~1e-15 и det-кодбук
            # становится ТОЧНЕЕ атомарного, а не «просто другим».
            cnts = torch.bincount(assign, minlength=K)
            order = torch.argsort(assign, stable=True)
            mu = X.mean(0).to(torch.float64)                        # (D_,) f64
            cs = torch.cumsum(X[order].to(torch.float64) - mu, 0)   # (N, D_) f64
            ends = torch.cumsum(cnts, 0)                            # (K,)
            starts = torch.cat([torch.zeros(1, dtype=torch.long, device=dev),
                                ends[:-1]])
            last = cs[(ends - 1).clamp(min=0)]
            first = cs[(starts - 1).clamp(min=0)]
            first = torch.where((starts == 0).unsqueeze(1),
                                torch.zeros_like(first), first)
            means = (last - first) / cnts.unsqueeze(1).to(torch.float64) + mu
            C[nz] = means[nz].to(C.dtype)
        else:
            sums = torch.zeros(K, D_, device=dev).index_add_(0, assign, X)
            cnts = torch.zeros(K, device=dev).index_add_(
                0, assign, torch.ones(N, device=dev))
