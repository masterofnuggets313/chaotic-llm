...
class PurePCLM(nn.Module):
    def __init__(self, vocab=512, d=128, layers=4, k_init=1.2, alpha=0.3,
        ...
        # ОТДЕЛЬНЫЙ АДРЕСНЫЙ ВЫХОД (§13.16, §13.17).
        # `q0` занят ридаутом (`readout3(cat([h_last, q0, g]))`), поэтому гнуть
        # его под внешнее адресное пространство = ломать языковую голову: CE
        # тянет в одну сторону, вспомогательная функция — в другую, CE побеждает
        # (измерено: cos_top1 вырос на 0.0026 за 1000 шагов при aux, которая
        # его прямо максимизирует). Здесь адрес = q0 + addr_b(q0), и addr_b
        # НИКУДА, кроме селекции, не попадает.
        # ПОСЛЕДНИЙ СЛОЙ — НУЛЯМИ. Тогда на старте addr_b(x) == 0, адрес в
        # точности q0, и старый чекпойнт остаётся совместим: шаг 0 сравнивается
        # с прежними прогонами напрямую, без поправок.
        self.addr_b = nn.Sequential(
            nn.Linear(d, d), nn.ReLU(), nn.Linear(d, d))
        nn.init.zeros_(self.addr_b[2].weight)
        nn.init.zeros_(self.addr_b[2].bias)
        self.topk = 8
        self.nquery = 4  # последние N токенов как multi-query для sts_prog
        self._last_sim = None  # для auxiliary loss (локализация повтора)

    def addr_of(self, q0):
        """Адрес селекции, ОТДЕЛЁННЫЙ от ридаута (§13.16).

        Ридаут по-прежнему получает СЫРОЙ `q0` (см. ветку `sts_prog`), а сюда
        уходит `q0 + addr_b(q0)`. При нулевой инициализации `addr_b` это
        тождество, так что старые чекпойнты ведут себя бит-в-бит по-прежнему.
        """
        if getattr(self, 'addr_b', None) is None:
            return q0
        return q0 + self.addr_b(q0)
    ...
    def forward(self, x, return_aux=False):
        ...
        if self.driver_mode in ("sts_prog", "sts_prog_nopc"):
            ...
            qa = self.addr_of(q0)      # адрес селекции; ридаут берёт сырой q0
            q = qa
